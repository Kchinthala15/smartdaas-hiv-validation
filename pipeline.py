...
from constants import (
    ...
    BASELINE_AUC, BASELINE_THRESHOLD, OUTCOME_NAME_HINTS,
    TRAINING_MEDIANS, CRITICAL_FEATURES, PEDIATRIC_AGE_CUTOFF,
)
...
def check_pediatric_patients(df_mapped):
    """
    Flag patients below the model's trained age range.

    The model is trained on adults 18-100 (see BASELINE_AUC note in constants.py).
    Patients under 18 are out of scope: paediatric WHO staging differs from adult
    staging, CD4 percentage is the paediatric marker rather than absolute count,
    and BMI is not interpretable without age-sex z-scores.

    Previously used `< 15`, which let 15-17 year olds through unflagged.
    """
    if 'Age' not in df_mapped.columns:
        return []
    try:
        ages = pd.to_numeric(df_mapped['Age'], errors='coerce')
        return list(df_mapped[ages < PEDIATRIC_AGE_CUTOFF].index)
    except Exception:
        return []
...
def validate_outcome_column(df, outcome_col, available_features):
    ...
    # Check 2b — Is this outcome plausibly ART adherence?
    # SmartDaaS predicts poor ART adherence at the most recent visit, which runs
    # at ~3.7% in the development cohort. A prevalence far from that means a
    # different construct has almost certainly been mapped (loss-to-follow-up,
    # mortality, a composite, or simply the wrong column). Recalibrating onto it
    # re-fits the deployed model to a different question, so this blocks.
    _prev = n_pos / n_clean if n_clean > 0 else 0
    _plausible = 0.005 <= _prev <= 0.25
    results['checks']['outcome_plausibility'] = {
        'passed': _plausible,
        'label': 'Outcome consistent with ART adherence',
        'detail': (
            f'Outcome prevalence is {_prev*100:.1f}%. SmartDaaS predicts poor ART '
            f'adherence, which runs at roughly 3-4% in programme data (3.67% in the '
            f'development cohort). A rate this high usually means a different outcome '
            f'has been mapped - loss-to-follow-up, mortality, or a composite - or that '
            f'the wrong column was selected. Recalibrating on it would fit the model to '
            f'a different question.'
            if _prev > 0.25 else
            f'Outcome prevalence is {_prev*100:.2f}% - fewer than 1 poor outcome per 200 '
            f'patients. Calibration on so few events is unstable.'
            if _prev < 0.005 else
            f'Outcome prevalence is {_prev*100:.1f}%, consistent with poor ART adherence '
            f'(3-4% expected).'
        ),
        'value': _prev,
        'threshold': '0.5%-25%',
    ...
    if n_missing > 0:
        results['warnings'].append(
            f'{n_missing:,} of {n_total:,} rows have an outcome value that could not be '
            f'mapped to poor/good and will be excluded. Check the outcome audit trail '
            f'for which values were unrecognised.'
        )

    # Overall decision
    blocking = ['sample_size', 'outcome_events', 'outcome_missingness',
                'outcome_definition', 'outcome_plausibility']
...
def sanitize_feature_matrix(df, features=None):
    ...
    for feat in features:
        ...
        if pd.api.types.is_numeric_dtype(col):
            df[feat] = pd.to_numeric(col, errors='coerce').fillna(TRAINING_MEDIANS[feat])
            continue
        ...
        try:
            ...
            df[feat] = TRAINING_MEDIANS[feat]
            audit_log.append(
                f"{feat}: could not convert to string — filled with the training median "
                f"({TRAINING_MEDIANS[feat]})")
            continue
        ...
        if not any(True for _ in non_numeric):
            ...
            df[feat] = pd.to_numeric(col_str, errors='coerce').fillna(TRAINING_MEDIANS[feat])
...
def run_recalibration(df_mapped, outcome_col, model, available_features):
    ...
    # Ensure all features present. Missing features get the TRAINING MEDIAN,
    # not 0 — 0 is the protective value for had_interruption, opp_infection,
    # tb_positive and stage_worsened, so zero-filling tells the model the
    # patient is fine and biases the whole recalibration.
    _imputed_feats = []
    for feat in FEATURES:
        if feat not in df_valid.columns:
            df_valid[feat] = TRAINING_MEDIANS[feat]
            _imputed_feats.append(feat)

    X = df_valid[FEATURES].values.astype(float)

    # ── Sanity guards on the outcome ─────────────────────────────────
    # The model predicts POOR ART ADHERENCE at the most recent visit,
    # prevalence ~3.7% in the development cohort. If the supplied outcome
    # is far from that, the most likely explanation is that a different
    # construct has been mapped (loss-to-follow-up, mortality, a composite,
    # or the wrong column entirely). Recalibrating onto the wrong outcome
    # does not just produce a bad number — it re-fits the deployed model
    # to predict something it was never built for.
    _prevalence = float(np.mean(y_true))
    _n_dropped = int((~valid_mask).sum())
    _guards = []
    if _prevalence > 0.25:
        _guards.append(
            f"Outcome prevalence is {_prevalence*100:.1f}%. SmartDaaS predicts poor ART "
            f"adherence, which runs at roughly 3-4% in programme data. A rate this high "
            f"usually means a different outcome has been mapped — loss-to-follow-up, "
            f"mortality, or a composite — or that the wrong column was selected. "
            f"Recalibrating on it would fit the model to a different question."
        )
    elif _prevalence < 0.005:
        _guards.append(
            f"Outcome prevalence is {_prevalence*100:.2f}%. That is fewer than 1 poor "
            f"outcome per 200 patients; calibration on so few events is unstable."
        )
    if _n_dropped > 0:
        _guards.append(
            f"{_n_dropped:,} of {len(df_mapped):,} rows were excluded because their outcome "
            f"value could not be mapped to poor/good. Check the audit trail below for which "
            f"values were unrecognised."
        )
    if _imputed_feats:
        _guards.append(
            f"{len(_imputed_feats)} feature(s) absent from this upload were filled with the "
            f"training-cohort median: {', '.join(_imputed_feats)}. Recalibration on imputed "
            f"features reflects the development cohort, not your programme."
        )
    ...
    return {
        ...
        # n_patients / n_positive / prevalence describe the FULL supplied cohort.
        # Every metric above (AUC, sensitivity, specificity, Brier, the confusion
        # matrix and the ROC curve) is computed on n_eval — the held-out 30%.
        'n_eval': int(len(y_true_test)),
        'n_eval_positive': int(y_true_test.sum()),
        'n_dropped': _n_dropped,
        'guards': _guards,
        'imputed_feats': _imputed_feats,
    }
...
def render_recalibration_page(model, log_recalibration=None, supabase=None):
    """
    Full Local Validation page UI.
    log_recalibration: optional callable for audit logging, passed in from app.py.
    supabase: optional supabase client for audit logging, passed in from app.py.
    """
    # Safe no-op if not provided
    if log_recalibration is None:
        def log_recalibration(*args, **kwargs): pass
    st.markdown("""
...
""")
    ...
    the number you report to funders, not the Nigerian discovery cohort baseline of 0.806.<br><br>
    ...
    with c1:
        st.markdown(
            '<div class="metric-box"><div class="metric-val">0.806</div>'
    ...
    st.markdown("---")
    # Keep the raw outcome values. normalize_outcome_column() replaces every
    # unrecognised value with NaN — which is how it becomes "ambiguous" — so by
    # the time the user is asked what `Transferred` means, the string is gone and
    # a mapping keyed on it can never match.
    _outcome_raw = df_recal[outcome_col_confirmed].copy()
    ...
    if _ambiguous:
        ...
        if _user_mappings:
            _col = df_recal[outcome_col_confirmed]
            _recoded = []
            # Walk the normalised column alongside the RAW one. A value that
            # normalise already resolved keeps its 0/1. A value that is NaN
            # because it was unrecognised is looked up by its ORIGINAL string.
            # A value that is NaN because the cell was genuinely blank stays NaN.
            for _raw_v, _v in zip(_outcome_raw, _col):
                if not pd.isna(_v):
                    _recoded.append(float(_v))
                elif not pd.isna(_raw_v) and str(_raw_v) in _user_mappings:
                    _recoded.append(float(_user_mappings[str(_raw_v)]))
                else:
...
def _render_recal_results(recal):
    ...
    st.markdown("---")
    # Data-quality guards, shown above the metrics every time the results are
    # rendered — not only on the run that produced them. A local AUC should
    # never be read without knowing what it was measured on.
    for _g in recal.get('guards', []):
        st.warning(f"⚠️ {_g}")
    ...
    with c2:
        st.markdown(
            ...
            f'<div class="metric-lbl">vs Baseline (0.806)</div></div>',
            unsafe_allow_html=True)
    with c3:
        st.markdown(
            f'<div class="metric-box">'
    ...
    ax.set_ylabel('True Positive Rate (Sensitivity)',
                  color='#8b949e', fontsize=9)
    ax.set_title(
        f'ROC Curve — Local Validation (held-out 30%)\n'
        f'(n={recal.get("n_eval", recal["n_patients"]):,} patients, '
        f'{recal.get("n_eval_positive", recal["n_positive"]):,} poor outcomes, '
    ...
    with col_left:
        st.markdown(f"""<div style="border:1px solid #3fb950;border-radius:8px;
            ...
            <tr><td style="padding:5px 6px;color:#8b949e">vs Baseline (0.806)</td>
    ...
    with col_right:
        st.markdown(f"""<div style="border:1px solid #444c56;border-radius:8px;
            ...
            <table style="width:100%;border-collapse:collapse;font-size:0.83rem">
            <tr><td style="padding:5px 6px;color:#8b949e;width:55%">Cohort supplied</td>
                <td style="padding:5px 6px;color:#cdd9e5">{recal['n_patients']:,} patients ({recal['n_positive']:,} poor outcomes, {recal['prevalence']*100:.1f}%)</td></tr>
            <tr><td style="padding:5px 6px;color:#8b949e">Calibrator fitted on</td>
                <td style="padding:5px 6px;color:#cdd9e5">{recal['n_patients'] - recal.get('n_eval', 0):,} patients (70%)</td></tr>
            <tr><td style="padding:5px 6px;color:#8b949e">Metrics measured on</td>
                <td style="padding:5px 6px;color:#e3b341">{recal.get('n_eval', 0):,} held-out patients ({recal.get('n_eval_positive', 0):,} poor outcomes)</td></tr>
...
def derive_engineered_features(df):
    ...
    Operates on a copy. Returns (df_out, list_of_derived, list_of_imputed).
    ...
    Any feature still missing after derivation is imputed with the TRAINING
    MEDIAN (constants.TRAINING_MEDIANS), which is the value the model's own
    internal imputer would use. It is NOT filled with 0.

    Why this matters: 0 is not a neutral value. For had_interruption,
    opp_infection, tb_positive and stage_worsened, 0 is the PROTECTIVE value —
    zero-filling tells the model the patient is fine. The third return value
    lists every feature that had to be imputed so the caller can decide whether
    scoring is safe. Features in constants.CRITICAL_FEATURES must not be
    imputed at all; the caller is expected to refuse to score.
    """
    df = df.copy()
    derived = []
    imputed = []
    ...
    # ── sex_female encoding ──────────────────────────────────
    # Unrecognised values -> NaN (imputed later), NOT 0.0 (= male).
    if 'sex_female' in df.columns:
        col = df['sex_female']
        if not pd.api.types.is_numeric_dtype(col):   # robust across pandas 1/2/3
            female_strings = {'f', 'female', 'woman', 'w', '1', 'yes', 'true'}
            male_strings   = {'m', 'male', 'man', '0', 'no', 'false'}
            def _enc_sex(x):
                if pd.isna(x):
                    return np.nan
                s = str(x).lower().strip()
                if s in female_strings:
                    return 1.0
                if s in male_strings:
                    return 0.0
                return np.nan
            df['sex_female'] = col.map(_enc_sex)

    # ── Binary categorical encoding ──────────────────────────
    # Missing/blank -> NaN. Only an explicit negative means 0.0.
    _NONE_STR = {'none', 'no', 'negative', '0', 'false'}
    _NULL_STR = {'nan', 'none_value', '', 'na', 'n/a', 'unknown', 'unk', '?'}

    def _encode_presence(x):
        # NaN must be tested BEFORE stringifying: on pandas >=3 a real NaN does
        # not become the string 'nan', so an `else 1.0` fallback would silently
        # turn a blank field into a recorded diagnosis.
        if pd.isna(x):
            return np.nan
        s = str(x).lower().strip()
        if s in _NULL_STR:
            return np.nan
        return 0.0 if s in _NONE_STR else 1.0

    for feat in ('opp_infection', 'side_effects'):
        if feat in df.columns:
            col = df[feat]
            if not pd.api.types.is_numeric_dtype(col):   # robust across pandas 1/2/3
                df[feat] = col.map(_encode_presence)

    # ── had_interruption / stage_worsened: Yes/No text ───────
    # These had no text encoder. had_interruption is the model's dominant
    # predictor (importance 0.381) and the source dataset records it as
    # 'Yes'/'No' text, so a programme exporting in the most natural format
    # had every patient silently coerced to NaN and then imputed to 0 —
    # "never interrupted", the protective value.
    _YES = {'yes', 'y', 'true', 't', '1', 'interrupted', 'interruption', 'positive'}
    _NO  = {'no', 'n', 'false', 'f', '0', 'none', 'never', 'negative', 'not interrupted'}
    ...
    for _feat in ('had_interruption', 'stage_worsened'):
        if _feat in df.columns:
            _col = df[_feat]
            if not pd.api.types.is_numeric_dtype(_col):
                df[_feat] = _col.map(_encode_yesno)

    if 'tb_positive' in df.columns:
        col = df['tb_positive']
        if not pd.api.types.is_numeric_dtype(col):   # robust across pandas 1/2/3
            pos_strings = {'yes', 'positive', '1', 'true', 'tb', 'tb positive', 'active'}
            neg_strings = {'no', 'negative', '0', 'false', 'none'}
            def _enc_tb(x):
                if pd.isna(x):
                    return np.nan
                s = str(x).lower().strip()
                ...
                if s in neg_strings:
                    return 0.0
                return np.nan
            df['tb_positive'] = col.map(_enc_tb)

    # ── who_stage / stage_start_num ──────────────────────────
    # Unparseable -> NaN. Previously defaulted to 2.0, an invented clinical value
    # that is also not the training median (which is 1.0).
    if 'stage_start_num' in df.columns:
        ...
        if not pd.api.types.is_numeric_dtype(col):   # robust across pandas 1/2/3
            ...
            def parse_stage(x):
                ...
                # take the last standalone digit 1-4, so "WHO 2014 stage 3" -> 3
                m = re.findall(r'\b([1-4])\b', s)
                if m:
                    return float(m[-1])
                return np.nan
    ...
    # Coerce to numeric but do NOT fill. Imputation happens once, at the end,
    # with the training medians.
    for feat in FEATURES:
        if feat in df.columns:
    ...
    # ── had_interruption ─────────────────────────────────────
    # CRITICAL FEATURE. If it cannot be derived it stays NaN and the caller
    # must refuse to score. Do not default it: 0 means "no prior interruption",
    # which is the protective value, and this is the model's dominant predictor.
    if 'had_interruption' not in df.columns:
        ...
        if interruption_col:
            n_int = pd.to_numeric(df[interruption_col], errors='coerce')
            df['had_interruption'] = (n_int >= 1).astype(float).where(n_int.notna())
            derived.append('had_interruption')
        else:
            df['had_interruption'] = np.nan
    ...
    if 'stage_worsened' not in df.columns:
        ...
        if current_stage_col and 'stage_start_num' in df.columns:
            ...
            df['stage_worsened'] = (stage_now > stage_start).astype(float).where(
                stage_now.notna() & stage_start.notna()
            )
            derived.append('stage_worsened')
        else:
            df['stage_worsened'] = np.nan

    # ── Any feature still absent -> create as NaN ────────────
    for feat in FEATURES:
        if feat not in df.columns:
            df[feat] = np.nan

    # ── Single imputation step, using the TRAINING medians ───
    # Anything still NaN here — whether the column was absent, blank, or
    # unparseable — gets the value the model was trained to expect.
    for feat in FEATURES:
