            ...
            try:
                ...
                for tei in instances:
                    ...
                    if enrollments:
                        ...
                        # Flatten all event data values — sort events by date
                        # so we always end up with the MOST RECENT value for
                        # each data element (critical for CD4, weight, interruption history)
                        events = latest.get('events', [])
                        events_sorted = sorted(
                            events,
                            key=lambda ev: ev.get('eventDate', ev.get('created', '')),
                            reverse=False   # ascending → latest event overwrites earlier ones
                        )
                        for event in events_sorted:
                            for dv in event.get('dataValues', []):
                                de_key = dv.get('dataElement', '').lower()
                                # Always overwrite — later events are more recent
                                row[de_key] = dv.get('value', '')

                        # Additionally capture prior_art_interruption from event history
                        # by checking if any historical event has an interruption flag
                        interruption_keys = {
                            k.lower() for k in row.keys()
                            if 'interrupt' in k.lower() or 'default' in k.lower()
                               or 'ltfu' in k.lower() or 'lost' in k.lower()
                        }
                        if interruption_keys:
                            # Check ALL events not just latest for any historical interruption
                            for event in events:
                                for dv in event.get('dataValues', []):
                                    de_key = dv.get('dataElement', '').lower()
                                    if any(ik in de_key for ik in ['interrupt', 'default', 'ltfu', 'lost']):
                                        val = dv.get('value', '')
                                        if val and str(val).strip() not in ('', '0', 'false', 'False', 'No', 'no'):
                                            row['prior_art_interruption_history'] = 1
                                            break
...
def map_dhis2_columns(df: pd.DataFrame, de_name_map: Dict[str, str]) -> pd.DataFrame:
    ...
    # Robust multi-format date parsing — DHIS2 uses various formats across countries
    date_cols_to_try = [
        ('art_start_date', 'date_hiv_diagnosis'),
        ('art_start_date', 'hiv_diagnosis_date'),
        ('art_start_date', 'diagnosis_date'),
        ('artstart', 'hivdiagnosis'),
    ]
    for art_col, diag_col in date_cols_to_try:
        if art_col in df.columns and diag_col in df.columns:
            try:
                # Try multiple date formats common in Nigerian/African DHIS2 instances
                ...
                art_dates = parse_dates_robust(df[art_col])
                hiv_dates = parse_dates_robust(df[diag_col])
                days = (art_dates - hiv_dates).dt.days
                # Sanity check: clip negative values and extreme outliers (>3650 days = 10 years)
                df['days_to_art'] = days.clip(lower=0, upper=3650)
                break
            except Exception:
                pass
    ...
    with st.expander("🔗 Connect directly to DHIS2 instance", expanded=False):
            ...
            if not all([base_url, username, password]):
                ...
                with st.spinner("Connecting to DHIS2..."):
                    ...
                    if ok:
                        ...
                        # Load programmes and org units — cache in session to avoid repeated API calls
                        with st.spinner("Loading programmes and facilities..."):
                            programs = client.get_programs()
                            st.session_state['dhis2_programs'] = programs
                            # Try facility level first (3), fall back to higher levels
                            org_units = client.get_org_units(level=3)
                            if not org_units:
                                org_units = client.get_org_units(level=2)
                            if not org_units:
                                org_units = client.get_org_units(level=4)
                            st.session_state['dhis2_org_units'] = org_units
                        st.info(f"Found {len(programs)} programme(s) and {len(org_units)} organisation unit(s)")
                    else:
                        st.error(f"❌ {msg}")
        ...
        if st.session_state.get('dhis2_connected'):
            ...
            max_records = st.slider(
                ...
                help="Larger pulls take longer. Start with 500-1000 to test. APIN cohorts may have 5,000-15,000 patients."
            )
            if max_records > 5000:
                st.warning(f"⚠️ Pulling {max_records:,} records may take 2-5 minutes depending on your DHIS2 server speed. Consider starting with 2,000 to test.")
            ...
            if st.button(
                ...
                if not selected_prog_id or not selected_ou_id:
                    ...
                    progress_bar = st.progress(0, text="Connecting to DHIS2...")
                    try:
                        progress_bar.progress(10, text="Fetching data element names...")
                        de_map = client.get_data_elements(selected_prog_id)

                        progress_bar.progress(25, text=f"Pulling patient records (up to {max_records:,})...")
                        df_raw, msg = client.get_tracked_entities(
                            program_id=selected_prog_id,
                            org_unit_id=selected_ou_id,
                            max_records=max_records
                        )
                        progress_bar.progress(80, text="Mapping columns to SmartDaaS format...")
                    except Exception as pull_err:
                        progress_bar.empty()
                        st.error(f"Pull failed: {str(pull_err)}")
                        df_raw, msg = pd.DataFrame(), str(pull_err)
                    ...
                    if df_raw.empty:
                        ...
                        st.info("💡 Tips: Check your programme UID is correct, your user has data access, and the org unit has enrolled patients.")
                    else:
                        ...
                        # Show column mapping preview — raw vs mapped
                        SMARTDAAS_CORE_COLS = [
                            ...
                            'bmi_start', 'art_start_date', 'prior_art_interruption_history'
                        ]
                        smartdaas_cols = [c for c in df_mapped.columns if c in SMARTDAAS_CORE_COLS]
                        unmapped = [
                            c for c in df_mapped.columns
                            if c not in SMARTDAAS_CORE_COLS and c != 'patient_id' and not c.startswith('_')
                        ]

                        col_a, col_b = st.columns(2)
                        with col_a:
                            st.markdown(f"**✅ Mapped to SmartDaaS ({len(smartdaas_cols)} fields)**")
                            if smartdaas_cols:
                                for c in smartdaas_cols:
                                    st.markdown(f"  • `{c}`")
                            else:
                                st.warning("No core fields mapped — check your DHIS2 field names")
                        with col_b:
                            st.markdown(f"**ℹ️ Additional fields ({len(unmapped)})**")
                            for c in unmapped[:8]:
                                st.markdown(f"  • `{c}`")
                            if len(unmapped) > 8:
                                st.markdown(f"  • ... and {len(unmapped)-8} more")

                        # Data preview
                        with st.expander("Preview first 5 rows", expanded=False):
                            preview_cols = ['patient_id'] + smartdaas_cols[:8]
                            preview_cols = [c for c in preview_cols if c in df_mapped.columns]
                            st.dataframe(df_mapped[preview_cols].head(5), use_container_width=True)
