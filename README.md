...
> ⚠️ **Synthetic data notice:** All performance metrics shown in the v2 architecture modules reflect synthetic architecture-testing environments only and are not representative of real-world clinical deployment performance. Prospective validation on longitudinal programme EMR data is planned as the primary objective of the APIN and AMPATH pilot studies.
...
- [Methodological Reporting](#methodological-reporting)
...
SmartDaaS adheres to international standards for prediction model development and reporting.

| Document | Description |
|---|---|
| 📋 [TRIPOD Checklist](SmartDaaS_TRIPOD_Checklist.docx) | Transparent Reporting of a Multivariable Prediction Model — all 21 items reported |
| 🔬 [PROBAST Assessment](SmartDaaS_PROBAST_Positioning.docx) | Prediction model Risk Of Bias Assessment — Low risk across all 4 domains |

> To our knowledge, SmartDaaS is among the first identified HIV treatment-retention prediction platforms combining multi-country external validation with calibration analysis, decision curve analysis, subgroup fairness assessment, per-patient explainability, and a live deployed platform — a methodological combination not previously identified in the published literature (Kwarah et al., 2025, BMC Global and Public Health).
...
SmartDaaS v1.0 is organised as a modular Python codebase. Each file has a single, auditable responsibility:

```
smartdaas-hiv-validation/
│
├── app.py              # Main entry point — page routing and session management
├── constants.py        # All model features, column aliases, tier definitions, thresholds
├── styles.py           # Platform CSS injected at startup
├── model.py            # Model loading, risk predictions, SHAP computation
├── pipeline.py         # Data ingestion, column mapping, DQ screening, feature engineering
├── outreach.py         # Capacity-constrained outreach optimiser engine
├── reports.py          # Executive PDF report generation
│
├── cv_results.pkl      # Trained Random Forest model + cross-validation results
├── prepped_data.pkl    # Demo cohort (500 synthetic patients)
├── requirements.txt    # Python dependencies
├── render.yaml         # Render deployment configuration
└── Dockerfile          # Container build
```

**For reviewers and auditors:** The prediction pipeline runs as `pipeline.py → model.py → outreach.py → reports.py`. Start with `constants.py` to understand all model inputs, then `model.py` for the core prediction logic.
...
```
Chinthala LK. SmartDaaS: HIV Programme Intelligence Platform (v1.0.0).
...
This project is licensed under the Creative Commons Attribution-NonCommercial-ShareAlike 4.0 International License (CC BY-NC-SA 4.0) — see [LICENSE](LICENSE) for details. Commercial use requires a separate written license from SmartDaaS LLC. Contact: lkchinthala@smartdaas.org
