...
[![License](https://img.shields.io/badge/License-PolyForm%20Noncommercial-8b949e)](LICENSE)
...
├── requirements-platform.txt   # what the app needs
├── requirements-research.txt   # v2 prototype deps — NOT the platform's
...
Dependencies are split in two: `requirements-platform.txt` builds the application; `requirements-research.txt` covers the v2 prototypes (`torch`, `econml`, `scikit-survival`) and is not needed to run SmartDaaS. A single earlier `requirements.txt` mixed the two and omitted `streamlit`, `shap`, `fpdf2`, `supabase`, `requests` and `openpyxl` — so it could not build the platform at all.
...
**Near-term:** external validation via CNICS / IeDEA West Africa · deploy `api.py` · resolve training-data provenance
...
**Free, with no agreement or fee**, for charitable organisations, educational institutions, public research organisations, health organisations and government institutions — **regardless of the source of funding**. Grant-funded implementation work is covered. Ministries, implementing partners, NGOs and academic groups can evaluate, deploy and build on SmartDaaS without contacting us.

**Commercial use** — integration into a for-profit product or service — requires a separate licence: lkchinthala@smartdaas.org

**The training data is not covered by this licence.** The *Quality of Care* dataset is a third-party deposit whose licence grants no rights ("Data files © Original Authors"). It is not redistributed in this repository and should not be redistributed from it. Rights in any model artefact derived from it are correspondingly unsettled.
