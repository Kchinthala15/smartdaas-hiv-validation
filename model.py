...
def _load_pickle(path):
    ...
    v1.1 stores cv_results.pkl gzipped (29.4 MB -> 9.5 MB) so it fits under
    GitHub's 25 MB web-upload limit. The filename is unchanged, so this sniffs
    the two-byte gzip magic number rather than relying on the extension. Plain
    pickles still load, so older artifacts keep working.
    """
    import gzip
...
def load_model():
    try:
        cv = _load_pickle('cv_results.pkl')
        prep = _load_pickle('prepped_data.pkl')
