...
def log_upload(supabase, n_patients, n_high, n_medium, avg_risk, source):
    """Log an upload event to Supabase. Fails silently if not configured."""
    if supabase is None:
        return
    try:
...
def log_report(supabase, org_name, n_patients, report_type):
    """Log a report generation event to Supabase. Fails silently if not configured."""
    if supabase is None:
        return
    try:
    ...
    with col2:
        ...
        if st.button("→ Enter Platform", use_container_width=True, type="primary"):
            if password_input == correct_password:
                ...
                st.error("Incorrect access code. Contact chinthalakalyani1@gmail.com for access.")
...
# ─────────────────────────────────────────────────────────────
# TIER DETECTION — inserted after normalize_columns
# ─────────────────────────────────────────────────────────────
...
def detect_outcome_columns(df):
    ...
    for col in df.columns:
        ...
        if col_lower in OUTCOME_NAME_HINTS:
            reason = f'column name matches known outcome identifier'

        # Value-pattern detection — binary 0/1 with reasonable prevalence
        if reason is None:
            try:
    ...
    candidates = [(c, r, v) for c, r, v in candidates
...
def validate_outcome_column(df, outcome_col, available_features):
    ...
    results['checks']['outcome_events'] = {
        ...
        'detail': (f'{n_pos:,} positive outcome events '
                   ...
                   f'Outcome prevalence: {n_pos/n_clean*100:.1f}%'),
        'value': n_pos,
...
def _render_recal_results(recal):
    ...
    st.markdown(f"""<div style="border:1px solid #3fb950;border-radius:8px;
        padding:20px;background:#0d2137;margin:12px 0">
        <div style="color:#3fb950;font-size:1rem;font-weight:700;margin-bottom:12px">
            ✅ SmartDaaS Local Validation Summary
        </div>
        <table style="width:100%;border-collapse:collapse;color:#cdd9e5;font-size:0.85rem">
        <tr><td style="padding:4px 8px;color:#8b949e">Validation cohort</td>
            <td style="padding:4px 8px">{recal['n_patients']:,} patients with known outcomes</td></tr>
        <tr><td style="padding:4px 8px;color:#8b949e">Outcome prevalence</td>
            <td style="padding:4px 8px">{recal['prevalence']*100:.1f}% poor outcome rate</td></tr>
        <tr><td style="padding:4px 8px;color:#8b949e">Local AUC</td>
            <td style="padding:4px 8px;color:#3fb950;font-weight:700">
            {recal['auc_cal']:.3f}{f" (95% CI: {recal['auc_ci_lo']:.3f}–{recal['auc_ci_hi']:.3f})" if recal['auc_ci_lo'] else ""}</td></tr>
        <tr><td style="padding:4px 8px;color:#8b949e">vs Nigerian baseline</td>
            <td style="padding:4px 8px">Local performance is {perf_interp} the Nigerian discovery cohort baseline (0.772)</td></tr>
        <tr><td style="padding:4px 8px;color:#8b949e">Sensitivity</td>
            <td style="padding:4px 8px">{recal['threshold']['sensitivity']*100:.1f}% of poor-outcome patients correctly identified</td></tr>
        <tr><td style="padding:4px 8px;color:#8b949e">Specificity</td>
            <td style="padding:4px 8px">{recal['threshold']['specificity']*100:.1f}% of good-outcome patients correctly classified</td></tr>
        <tr><td style="padding:4px 8px;color:#8b949e">Calibration quality</td>
            <td style="padding:4px 8px">Brier score {recal['brier']:.3f} — {brier_interp} calibration</td></tr>
        <tr><td style="padding:4px 8px;color:#8b949e">Calibration method</td>
            <td style="padding:4px 8px">{recal['cal_method']}</td></tr>
        </table>
        <div style="margin-top:12px;color:#8b949e;font-size:0.75rem">
        ⚠️ This validation used historical retrospective data from your programme.
        Prospective validation is recommended before full operational deployment.
        All outputs require review by qualified programme and clinical staff.
        </div>
    </div>""", unsafe_allow_html=True)
...
with st.sidebar:
    ...
    if MODEL_OK:
        ...
        <div class="metric-box">
            <div class="metric-val">0.772</div>
            <div class="metric-lbl">Temporal AUC (realistic)</div>
        </div>""", unsafe_allow_html=True)
        st.markdown(f"""
        <div style="color:#8b949e;font-size:0.7rem;text-align:center;margin-top:4px">
            Cross-val AUC: {MODEL_AUC:.3f} (optimistic)
        </div>""", unsafe_allow_html=True)
...
if page == "🏠 Home":
    ...
    <div style="background:#0d2137;border:1px solid #21d4fd;border-radius:6px;
        ...
        <span style="color:#21d4fd;font-family:'IBM Plex Mono',monospace;font-size:0.75rem;
            font-weight:700">v1.1.0 — 15 May 2026</span>
        <span style="color:#8b949e;font-size:0.75rem">
            International tier detection · IeDEA MUD benchmarks ·
            Data quality screening · Honest AUC framing · Paediatric flags
        </span>
    </div>""", unsafe_allow_html=True)
    ...
    st.markdown("""<div style="background:linear-gradient(135deg,#0d1f17,#0d2137);
        ...
        go to <strong>📊 Patient Risk</strong> in the sidebar</p>
        <p style="color:#adbac7;font-size:0.85rem;margin:0">
        Or see the <strong>🤝 Pilot Model</strong> page to learn about our
...
> Adherence Prediction and Care Gap Quantification: Validation on 27,288 Nigerian HIV Programme Records with Temporal Holdout, SHAP Explainability, and Decision Curve Analysis.* 2026.
...
*DOIs will be added upon publication.*
...
""")
    ...
    st.markdown("""<div style="background:#161b22;border:1px solid #444c56;
        ...
        Independent researcher and graduate of the Ageno School of Business,
        Golden Gate University, San Francisco, CA. Specialising in machine learning
    ...
    if MODEL_OK:
        ...
        with c_top:
            ...
            top5.index = [f"PT-{i:04d}" for i in range(len(top5))]
            top5.columns = ['Risk %', 'Tier']
            top5['Tier'] = top5['Tier'].map({'HIGH':'🔴 HIGH','MEDIUM':'🟡 MED','LOW':'🟢 LOW'})
            st.dataframe(top5, use_container_width=True, height=210)
    else:
        st.info("Model loading... refresh in a moment.")
...
""")
    ...
    st.markdown("""
> **Paper 1 — Under review at npj Digital Medicine**
> Chinthala LK. *Real-World Validation of Machine Learning Models for HIV Treatment Adherence
> Prediction and Care Gap Quantification: Validation on 27,288 Nigerian HIV Programme Records (Discovery Cohort).* 2026.
> 📎 Code: [github.com/Kchinthala15/smartdaas-hiv-validation](https://github.com/Kchinthala15/smartdaas-hiv-validation)
...
> and Global Fund Programming.* 2026.

*DOIs will be added upon publication. Analysis code openly available on GitHub.*
...
""")
    ...
    st.markdown("""<div style="background:#0d1f17;border:1px solid #3fb95044;
        ...
        🔒 Privacy by Design</p>
        <p style="color:#cdd9e5;font-size:0.875rem;margin:0;line-height:1.6">
        <strong>No data storage:</strong> Patient data uploaded to SmartDaaS is processed
        entirely within your browser session and is never stored, transmitted to external
        servers, or retained after your session ends.<br><br>
        <strong>No third-party sharing:</strong> Your programme data is not shared with
        any third party, including the developer.<br><br>
        <strong>Session-only processing:</strong> All risk scores, SHAP explanations,
        and reports are generated in real time and exist only during your active session.<br><br>
        <strong>For pilot deployments:</strong> A formal Data Use Agreement (DUA) will be
        established with all pilot partners before any programme data is ingested.
        All pilots operate under applicable data protection regulations including
        Nigeria's NDPR and relevant PEPFAR data governance frameworks.<br><br>
        <strong>Platform notice:</strong> SmartDaaS v1.0 is a decision-support platform
        designed for HIV programme intelligence and operational analytics. It is not intended
        to replace clinical judgment. All outputs should be reviewed and validated by
        qualified programme teams prior to operational use.
        patient treatment decisions without clinical review and appropriate regulatory approval.
        </p>
    </div>""", unsafe_allow_html=True)
    ...
    with col1:
        st.markdown("""### Interested in a Shadow Analytics Pilot?
SmartDaaS is seeking one implementing partner for a 6-month analytical pilot.
You provide historical programme data. We deliver patient risk intelligence
and facility performance reports.

📧 **chinthalakalyani1@gmail.com**
🔗 [github.com/Kchinthala15/smartdaas-hiv-validation](https://github.com/Kchinthala15/smartdaas-hiv-validation)
👉 See the **🤝 Pilot Model** page for full details
""")
    with col2:
        st.markdown("""<div class="warn-box">
⚠️ <strong>Decision-support platform — not a clinical tool.</strong>
SmartDaaS v1.0 is designed for HIV programme intelligence and operational analytics.
It is not intended to replace clinical judgment or function as an autonomous clinical
decision-making system. All outputs should be reviewed by qualified programme teams.
Local validation is required before deployment in real-world programme environments.
</div>""", unsafe_allow_html=True)
...
""")
    ...
    if uploaded is not None and not use_demo:
        try:
            ...
            # ── Step 7: Fill missing Enhanced features with neutral default ─
            if tier in ('STANDARD', 'CORE'):
                for feat in FEATURES:
                    if feat not in df_mapped.columns:
                        df_mapped[feat] = 0
            ...
            log_upload(supabase, n_patients, 0, 0, 0.0, uploaded.name)
    ...
    if uploaded is not None and not use_demo:
        log_upload(supabase, n_tot, n_high, n_med,
                   float(df_input['risk_pct'].mean()), uploaded.name)
...
> 27,288 patients in the Nigerian national HIV programme (Paper 2, under preparation).
...
> Chinthala LK. *Real-World Validation of Machine Learning Models for HIV Treatment
Adherence Prediction and Care Gap Quantification: Validation on 27,288 Nigerian HIV Programme Records (Discovery Cohort).* Under review at npj Digital Medicine, 2026.

> Chinthala LK. *Facility-Level Structural Drivers of HIV Treatment Outcomes: A Multi-Level
Analysis of 27,288 Patients from a Nigerian HIV Programme and Implications for PEPFAR
and Global Fund Programming.* Under preparation for submission, 2026.
...
""")
    ...
    col_up, col_demo = st.columns([2, 1])
    with col_up:
        uploaded_rep = st.file_uploader("Upload patient CSV for report", type=['csv'])
    with col_demo:
        st.markdown("<br>", unsafe_allow_html=True)
        use_demo_rep = st.checkbox("Use demo data", value=(uploaded_rep is None))
    ...
    if use_demo_rep or uploaded_rep is None:
        ...
        try:
            ...
            # Standard tier: fill missing features and disclose
            if rep_tier == 'STANDARD':
                for feat in FEATURES:
                    if feat not in df_rep.columns:
                        df_rep[feat] = 0
                st.warning(
    ...
    if st.button("📄 Generate Executive PDF Report", use_container_width=True, type="primary"):
        with st.spinner("Generating professional PDF report..."):
            try:
                ...
                pdf.cell(0, 6, f'Report Date: {report_date.strftime("%d %B %Y")}', align='C', ln=True)
                pdf.cell(0, 6, f'Prepared by: {prepared_by}', align='C', ln=True)
                pdf.cell(0, 6, f'Data source: {data_source}', align='C', ln=True)
                ...
                for label, value in metrics:
                    ...
                    pdf.cell(110, 7, f'  {label}', border=0, fill=True)
                    pdf.set_text_color(33, 212, 253)
                ...
                findings = [
                    ...
                    f'{pct_low_cd4:.1f}% had CD4 <200 cells/µL at ART start — severely immunosuppressed',
                ]
                if pct_tb > 5:
                ...
                pdf.set_font('Helvetica', '', 9)
                pdf.set_text_color(173, 186, 199)
                pdf.multi_cell(0, 5, 'Based on Paper 2 analysis of 27,288 patients from the Nigerian national HIV programme (Chinthala 2026, under preparation for submission). Findings are directionally consistent with evidence from South Africa (Bor et al. 2021, PLOS Medicine).')
                pdf.ln(4)
                ...
                for finding_title, stat, desc, action in facility_findings:
                    ...
                    pdf.set_font('Helvetica', 'B', 9)
                    pdf.set_text_color(33, 212, 253)
                    pdf.cell(0, 7, f'  {finding_title}: {stat}', fill=True, ln=True)
                    pdf.set_font('Helvetica', '', 8.5)
                    pdf.set_text_color(230, 237, 243)
                    pdf.multi_cell(0, 5, f'  Finding: {desc}')
                ...
                pdf.set_text_color(33, 212, 253)
                cols = [('Patient ID', 28), ('Risk Score', 22), ('Age', 15),
                        ('CD4 Start', 22), ('WHO Stage', 22), ('Interruption', 22), ('Action', 54)]
                ...
                for _, row in top10.iterrows():
                    ...
                    cells = [
                        (str(row['patient_id']), 28),
                        (f"{row['risk_pct']:.1f}%", 22),
                        (f"{row['Age']:.0f}", 15),
                        (f"{row['Cd4AtStart']:.0f}", 22),
                        (f"Stage {row['stage_start_num']:.0f}", 22),
                        ("Yes" if row['had_interruption'] > 0.5 else "No", 22),
                        (urgency, 54),
                    ]
                ...
                log_report(supabase, org_name, n_total, "Executive PDF")
                st.download_button(
...
""")
    ...
    decision-making and should not be used to make individual patient treatment decisions
    without clinical review, local validation, and appropriate regulatory oversight.
    </div>""", unsafe_allow_html=True)
...
# ═════════════════════════════════════════════════════════════
    ...
    if supabase is not None:
        try:
            ...
            if result.data:
                uploads_df = pd.DataFrame(result.data)
                uploads_df['uploaded_at'] = pd.to_datetime(
                    uploads_df['uploaded_at']).dt.strftime('%Y-%m-%d %H:%M')
                display_cols = ['uploaded_at', 'data_source', 'n_patients',
                                'n_high_risk', 'n_medium_risk', 'avg_risk_score']
                ...
                # Stats
                c1, c2, c3 = st.columns(3)
                with c1: st.metric("Total uploads", len(uploads_df))
                with c2: st.metric("Total patients analysed",
                                   f"{uploads_df['n_patients'].sum():,}" if 'n_patients' in uploads_df.columns else "—")
                with c3: st.metric("Unique data sources",
                                   uploads_df['data_source'].nunique() if 'data_source' in uploads_df.columns else "—")
            else:
                st.info("No uploads recorded yet.")
        except Exception as e:
            st.warning(f"Could not fetch upload history: {e}")
    else:
        st.info("Connect Supabase to see upload history.")

    # ── REPORT HISTORY ────────────────────────────────────
    st.markdown('<p class="section-hdr">Report History</p>', unsafe_allow_html=True)

    if supabase is not None:
        try:
            result = supabase.table("reports").select("*").order(
                "generated_at", desc=True).limit(50).execute()
            if result.data:
                reports_df = pd.DataFrame(result.data)
                reports_df['generated_at'] = pd.to_datetime(
                    reports_df['generated_at']).dt.strftime('%Y-%m-%d %H:%M')
