...
# ═════════════════════════════════════════════════════════════
    ...
    with c3:
        ...
        for i in top3_idx:
            ...
            direction = "increases" if float(pt_sv[ii]) > 0 else "reduces"
            drivers.append(f"**{feat_name}** {direction} risk (SHAP: {float(pt_sv[ii]):+.3f})")
    ...
    sv_labels = []
    for i, name in zip(sv_order, sv_names):
        ii = int(i)
        val = sel_row[feat_list2[ii]]
        sv_labels.append(f"{name} = {float(val):.1f}")
