...
DUA_STATEMENTS = [
    ...
    },
    {
        ...
        "text": (
            ...
            "session is processed in-browser only and is not stored, "
            "transmitted externally, or retained after this session ends."
        ),
    },
]
...
def run_predictions(df_in):
    # Safety net: ensure every model feature exists before indexing.
    # derive_engineered_features should have run already, but this guards
    # against any remaining edge case.
    df_in = df_in.copy()
    for f in FEATURES:
...
if page == "🏠 Home":
    ...
    <div style="background:#0d2137;border:1px solid #21d4fd;border-radius:6px;
        ...
        <span style="color:#21d4fd;font-family:'IBM Plex Mono',monospace;font-size:0.75rem;
            font-weight:700">v1.2.0 — 15 May 2026</span>
...
""")
    ...
    # ── DUA gate — shown before upload widget ─────────────
    # User must acknowledge data governance before any file can be selected.
    st.markdown('<p class="section-hdr">Data Governance</p>', unsafe_allow_html=True)
    dua_ok = render_dua_gate(context="upload")

    if not dua_ok:
        st.info(
            "👆 Please acknowledge all three statements above before uploading patient data. "
            "You can also check **🔬 Use demo data** below to explore the platform without uploading."
        )
        # Still allow demo data without DUA
        ...
        if not use_demo_only:
            st.stop()
        # Proceed with demo data path
        uploaded = None
        use_demo = True
    else:
        # ── Upload / demo toggle — shown after DUA ─────────
...
""")
    ...
    # ── DUA gate — shown before upload widget ─────────────
    st.markdown('<p class="section-hdr">Data Governance</p>', unsafe_allow_html=True)
    rep_dua_ok = render_dua_gate(context="report")

    if not rep_dua_ok:
        st.info(
            "👆 Please acknowledge all three statements above before uploading patient data. "
            "You can also use demo data to preview the report format without uploading."
        )
        st.markdown('<p class="section-hdr">Or preview with demo data</p>', unsafe_allow_html=True)
        use_demo_rep_only = st.checkbox(
            "🔬 Use demo data to preview report format",
            value=False,
            help="300 patients from the training set — no upload required"
        )
        if not use_demo_rep_only:
            st.stop()
        uploaded_rep = None
        use_demo_rep = True
    else:
        # ── DATA INPUT — shown after DUA ──────────────────
    ...
    # ── Run predictions ───────────────────────────────────
    # Safety net: ensure all features present (derive_engineered_features
    # already ran for uploads; demo data has all features by construction)
    for f in FEATURES:
