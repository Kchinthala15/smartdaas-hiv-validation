...
if page == "🏠 Home":
    ...
    st.markdown("""<div style="background:linear-gradient(135deg,#0d1117 0%,#161b22 50%,#0d2137 100%);
        ...
        and economic insights into one powerful platform. Built on real-world data from
        <strong style="color:#21d4fd">192,732 HIV patient records</strong>, it delivers
        actionable intelligence that helps programmes improve adherence, reduce treatment
        interruptions, and optimise resources.</p>
    </div>""", unsafe_allow_html=True)
...
""")
    ...
    st.markdown("""<div style="background:#161b22;border:1px solid #444c56;
        ...
        Independent researcher and graduate student at the Ageno School of Business,
...
> Prediction and Care Gap Quantification: A Multi-Country Analysis of 192,732 Clinical Records.* 2026.
> 📎 Preprint: [github.com/Kchinthala15/smartdaas-hiv-validation](https://github.com/Kchinthala15/smartdaas-hiv-validation)
...
""")
    ...
    if uploaded is not None and not use_demo:
        try:
            ...
            log_upload(supabase, len(df_input), 0, 0, 0.0, uploaded.name)
...
projections — not guaranteed savings.
    ...
    if preset == "nigeria":
        default_total=27288; default_primary=2; default_secondary=66; default_cost=880; default_tertiary=10
        st.markdown("""<div class="success-box">✓ <strong>Nigeria Historical Baseline:</strong>
        Parameters from Paper 2. 27,288 patients; 47.3% experienced diagnosis-to-ART delays >90 days;
        primary HC poor outcome 15.4% vs 10.2% tertiary. Cost: Menzies et al. 2011.</div>""",
        unsafe_allow_html=True)
    elif preset == "conservative":
        default_total=5000; default_primary=5; default_secondary=60; default_cost=231; default_tertiary=10
        ...
        default_total=27288; default_primary=10; default_secondary=66; default_cost=880; default_tertiary=8
...
""")
    ...
    if use_demo_rep or uploaded_rep is None:
        ...
        data_source = "Demo dataset (200 patients from training set)"
    else:
        try:
            df_raw = pd.read_csv(uploaded_rep)
            df_rep, missing, _ = normalize_columns(df_raw)
            if missing:
                st.error(f"Missing columns: {missing}")
                st.stop()
            if 'patient_id' not in df_rep.columns:
                df_rep['patient_id'] = [f"PT-{i:04d}" for i in range(len(df_rep))]
            data_source = f"Uploaded dataset ({len(df_rep):,} patients)"
        except Exception as e:
            st.error(f"Could not read file: {e}")
            st.stop()
    ...
    if st.button("📄 Generate Executive PDF Report", use_container_width=True, type="primary"):
        with st.spinner("Generating professional PDF report..."):
            try:
                ...
                pdf.set_font('Helvetica', '', 9)
                pdf.set_text_color(173, 186, 199)
                pdf.multi_cell(0, 5, 'Based on Paper 2 analysis of 27,288 patients from the Nigerian national HIV programme (Chinthala 2026, under review BMJ Global Health). Findings are directionally consistent with evidence from South Africa (Bor et al. 2021, PLOS Medicine).')
                pdf.ln(4)
                ...
                pdf.set_text_color(173, 186, 199)
                methodology_text = (
                    "Patient risk scores are generated by a Random Forest classifier trained on 192,732 HIV patient records "
                    "from the CEPHIA multi-country dataset. Cross-validation AUC: 0.963; temporal validation AUC: 0.772. "
...
""")
    ...
    The model was trained on the CEPHIA multi-country dataset (192,732 records, 2006–2018).
