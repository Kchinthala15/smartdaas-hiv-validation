...
""")
    ...
    # -- DUA gate --------------------------------------------------
    # Show checkboxes only if not yet acknowledged this session.
    # Once acknowledged, show only compact green banner - never block again.
    if not is_dua_acknowledged():
        ...
        render_dua_gate(context="upload")  # compact green banner only
        uploaded = None
        use_demo = False

    # -- Upload widget (shown once DUA is acknowledged) -------------
    # Stored in session_state so the file survives rerenders.
    if is_dua_acknowledged() and not (locals().get('use_demo') or False):
        ...
        # Persist file across rerenders
        if uploaded_widget is not None:
            st.session_state['risk_uploaded_file'] = uploaded_widget
        if use_demo:
            st.session_state.pop('risk_uploaded_file', None)
        uploaded = None if use_demo else st.session_state.get('risk_uploaded_file', None)
...
Adherence Prediction and Care Gap Quantification: A Multi-Country Analysis of 192,732 Clinical Records.* Under review at npj Digital Medicine, 2026.
...
""")
    ...
    if not is_dua_acknowledged():
        ...
        if not rep_dua_ok:
            st.info("Please acknowledge all three statements above to unlock upload and report generation.")
            st.stop()
    else:
        render_dua_gate(context="report")  # compact green banner only
    ...
    col_up, col_demo = st.columns([2, 1])
    with col_up:
        uploaded_rep_widget = st.file_uploader(
    ...
    # Persist file across rerenders
    if uploaded_rep_widget is not None:
        st.session_state['rep_uploaded_file'] = uploaded_rep_widget
    if use_demo_rep:
        st.session_state.pop('rep_uploaded_file', None)
    uploaded_rep = None if use_demo_rep else st.session_state.get('rep_uploaded_file', None)
    # Report metadata
