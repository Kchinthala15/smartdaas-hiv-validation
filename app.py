...
.metric-val { font-family:'IBM Plex Mono',monospace; font-size:1.8rem;
    font-weight:600; color:#21d4fd; white-space:nowrap; overflow:hidden;
    text-overflow:ellipsis; display:block; }
.metric-lbl { font-size:0.75rem; color:#adbac7; text-transform:uppercase;
...
with st.sidebar:
    ...
    def nav_group(label):
        st.markdown(
            f'<div style="font-size:0.65rem;color:#484f58;text-transform:uppercase;'
            f'letter-spacing:1.5px;padding:8px 0 2px 4px;margin-top:4px">{label}</div>',
            unsafe_allow_html=True
        )

    nav_group("Overview")
    page_overview = st.radio("", [
        "🏠 Home",
        "📊 Programme Dashboard",
    ], label_visibility="collapsed", key="nav_overview")

    nav_group("Patient Analytics")
    page_patient = st.radio("", [
        "📊 Patient Risk",
        "🧠 SHAP Explainability",
        "🎯 Intervention Engine",
    ], label_visibility="collapsed", key="nav_patient")

    nav_group("Programme Intelligence")
    page_programme = st.radio("", [
        "🏥 Facility Intelligence",
        "📈 Cohort Intelligence",
        "💰 Economic Calculator",
        "📄 Executive Report",
    ], label_visibility="collapsed", key="nav_programme")

    nav_group("Validation & Model")
    page_model = st.radio("", [
        "🔬 Model Transparency",
        "🔬 Local Validation",
        "🤝 Pilot Model",
        ...
        "📋 Sample Data",
        "🔐 Admin",
    ], label_visibility="collapsed", key="nav_model")

    # Resolve active page — whichever group was most recently changed
    _all_pages = [
        page_overview, page_patient, page_programme, page_model
    ]
    # Track last selected using session state
    _current_selections = {
        "nav_overview": page_overview,
        "nav_patient": page_patient,
        "nav_programme": page_programme,
        "nav_model": page_model,
    }
    if "last_nav_group" not in st.session_state:
        st.session_state["last_nav_group"] = "nav_overview"
        st.session_state["last_nav_values"] = _current_selections.copy()

    for grp, val in _current_selections.items():
        if val != st.session_state["last_nav_values"].get(grp):
            st.session_state["last_nav_group"] = grp
            st.session_state["last_nav_values"] = _current_selections.copy()
            break

    page = _current_selections[st.session_state["last_nav_group"]]
...
if page == "🏠 Home":
    ...
    st.markdown("""
    <div style="background:#0d2137;border:1px solid #21d4fd33;border-radius:6px;
        padding:6px 16px;margin-bottom:1rem;display:inline-flex;align-items:center;gap:12px">
        <span style="color:#21d4fd;font-family:'IBM Plex Mono',monospace;font-size:0.75rem;
            font-weight:700">v1.0 · Decision-Support Platform · Pilot-Ready</span>
    </div>""", unsafe_allow_html=True)
    ...
    if st.button("📄 Generate Executive PDF Report", use_container_width=True, type="primary"):
        with st.spinner("Generating professional PDF report..."):
            try:
                ...
                pdf.set_y(45)
                pdf.cell(0, 14, 'SmartDaaS', align='C', ln=True)

                pdf.set_font('Helvetica', '', 16)
                pdf.set_text_color(230, 237, 243)
                pdf.ln(2)
                pdf.cell(0, 10, 'HIV Programme Intelligence Report', align='C', ln=True)

                pdf.set_font('Helvetica', '', 12)
                pdf.set_text_color(173, 186, 199)
                pdf.ln(10)
                pdf.cell(0, 8, org_name, align='C', ln=True)
                pdf.cell(0, 8, programme_name, align='C', ln=True)
                pdf.ln(2)
                pdf.cell(0, 8, _s(f'Report Date: {report_date.strftime("%d %B %Y")}'), align='C', ln=True)
                pdf.cell(0, 8, _s(f'Prepared by: {prepared_by}'), align='C', ln=True)
                pdf.ln(4)
                pdf.set_font('Helvetica', '', 10)
                pdf.cell(0, 7, _s(f'Data source: {data_source}'), align='C', ln=True)
                ...
                pdf.multi_cell(180, 7, 'IMPORTANT: SmartDaaS v1.0 is a decision-support platform for HIV programme intelligence. It is not intended to replace clinical judgment. All outputs should be reviewed by qualified programme teams before operational use. Local validation is required before deployment.')
                ...
                pdf.set_text_color(230, 237, 243)
                pdf.cell(0, 9, 'Programme Risk Overview', ln=True)
                pdf.ln(3)
                ...
                for label, value in metrics:
                    ...
                    pdf.set_text_color(173, 186, 199)
                    pdf.set_font('Helvetica', '', 12)
                    pdf.cell(120, 9, _s(f'  {label}'), border=0, fill=True)
                    pdf.set_text_color(33, 212, 253)
                    pdf.set_font('Helvetica', 'B', 12)
                    pdf.cell(60, 9, value, border=0, fill=True, ln=True)
                    pdf.ln(1)

                pdf.ln(4)
                pdf.set_font('Helvetica', 'B', 12)
                pdf.set_text_color(230, 237, 243)
                pdf.cell(0, 9, 'Key Clinical Findings', ln=True)
                ...
                for finding in findings:
                    ...
                    pdf.set_text_color(230, 237, 243)
                    pdf.set_x(pdf.l_margin)
                    pdf.cell(7, 8, chr(149), ln=False)
                    pdf.multi_cell(163, 8, finding)
                    pdf.ln(2)
                ...
                pdf.set_font('Helvetica', 'B', 16)
                pdf.set_text_color(33, 212, 253)
                pdf.cell(0, 12, 'FACILITY INTELLIGENCE', ln=True)
                pdf.set_draw_color(33, 212, 253)
                pdf.line(15, pdf.get_y(), 195, pdf.get_y())
                pdf.ln(6)
                ...
                pdf.set_text_color(173, 186, 199)
                pdf.set_x(pdf.l_margin)
                pdf.multi_cell(180, 7, 'Based on Paper 2 analysis of 27,288 patients from the Nigerian national HIV programme (Chinthala 2026, submitted to BMJ Global Health). Findings are directionally consistent with evidence from South Africa (Bor et al. 2021, PLOS Medicine).')
                pdf.ln(4)
                ...
                facility_findings = [
                    ('Primary HC vs Tertiary Hospital', 'OR 1.95 (95% CI 1.45-2.61)',
                     ...
                     'Structural quality improvement investment - staffing, drug supply, monitoring systems.'),
                    ('NGO-Funded Facilities', 'OR 1.24 (95% CI 1.10-1.39)',
                     'NGO-funded facilities show independently higher odds - may reflect higher case complexity or administrative burden.',
                     'Outcome-adjusted performance monitoring; investigate funding-to-quality translation mechanisms.'),
                    ...
                    ('Female Sex - Protective Effect', 'OR 0.87 (95% CI 0.79-0.96)',
                     'Female sex is independently protective overall, but this advantage nearly disappears at primary health centres.',
                     'Male-targeted interventions at secondary/tertiary; structural improvements at primary HCs benefit all.'),
                ]
                ...
                for finding_title, stat, desc, action in facility_findings:
                    ...
                    pdf.set_text_color(33, 212, 253)
                    pdf.set_x(pdf.l_margin)
                    pdf.cell(180, 9, _s(f'  {finding_title}: {stat}'), fill=True, ln=True)
                    pdf.ln(1)
                    pdf.set_font('Helvetica', '', 12)
                    pdf.set_text_color(230, 237, 243)
                    pdf.set_x(pdf.l_margin)
                    pdf.multi_cell(180, 7, _s(f'  Finding: {desc}'))
                    pdf.set_text_color(63, 185, 80)
                    pdf.set_x(pdf.l_margin)
                    pdf.multi_cell(180, 7, _s(f'  Action: {action}'))
                    pdf.ln(5)
                ...
                pdf.set_font('Helvetica', 'B', 16)
                pdf.set_text_color(33, 212, 253)
                pdf.cell(0, 12, 'TOP 10 HIGHEST RISK PATIENTS', ln=True)
                pdf.set_draw_color(33, 212, 253)
                pdf.line(15, pdf.get_y(), 195, pdf.get_y())
                pdf.ln(3)
                ...
                pdf.set_text_color(173, 186, 199)
                pdf.cell(0, 8, 'Patients requiring immediate contact and adherence support.', ln=True)
                pdf.ln(3)
                ...
                pdf.set_text_color(33, 212, 253)
                cols = [('Patient ID', 30), ('Risk %', 20), ('Age', 14),
                        ('CD4', 20), ('Stage', 20), ('Interruption', 24), ('Action', 52)]
                for col_name, width in cols:
                    pdf.cell(width, 10, col_name, border=0, fill=True)
                pdf.ln()
                pdf.ln(1)
                ...
                pdf.set_font('Helvetica', 'B', 16)
                pdf.set_text_color(33, 212, 253)
                pdf.cell(0, 12, 'RECOMMENDED PROGRAMME ACTIONS', ln=True)
                pdf.set_draw_color(33, 212, 253)
                pdf.line(15, pdf.get_y(), 195, pdf.get_y())
                pdf.ln(6)
                ...
                pdf.set_font('Helvetica', 'B', 16)
                pdf.set_text_color(33, 212, 253)
                pdf.cell(0, 12, 'METHODOLOGY & LIMITATIONS', ln=True)
                pdf.set_draw_color(33, 212, 253)
                pdf.line(15, pdf.get_y(), 195, pdf.get_y())
                pdf.ln(6)
                ...
                pdf.set_x(pdf.l_margin)
                pdf.multi_cell(180, 8, methodology_text)
