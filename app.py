...
.brand-name { font-family:'IBM Plex Mono',monospace; font-size:2.2rem;
    font-weight:600; color:#00e5ff; letter-spacing:-1px; margin:0; }
.brand-sub { font-size:0.95rem; color:#b0bec5; margin-top:0.25rem; font-weight:300; }
...
.metric-lbl { font-size:0.75rem; color:#8fa0b0; text-transform:uppercase;
    letter-spacing:0.5px; white-space:normal; word-break:break-word; }
        ...
        with st.spinner("Generating professional PDF report..."):
            try:
                ...
                exec_metrics = [
                    ...
                    ('Estimated Avoidable Programme Cost',   f'USD {est_avoidable_cost:,}',            _CYAN),
                ]
                ...
                pdf.set_fill_color(*_CARD)
                pdf.set_font('Helvetica', 'B', 10); pdf.set_text_color(*_CYAN)
                t_cols = [('Patient ID',30),('Risk %',22),('Age',14),('CD4',20),('Stage',20),('Prior Int.',24),('Action',46)]
                pdf.set_x(15)
                ...
                for _, row in top10.iterrows():
                    ...
                    cells = [
                        ...
                        (f"{row['risk_pct']:.1f}%", 22, _RED),
                        (f"{row['Age']:.0f}", 14, _TEXT),
                        (f"{row['Cd4AtStart']:.0f}", 20, _TEXT),
                        (f"Stage {row['stage_start_num']:.0f}", 20, _TEXT),
                        ("Yes" if row['had_interruption'] > 0.5 else "No", 24, _TEXT),
                        (urgency, 46, _AMBER),
                    ]
                    pdf.set_x(15)
                ...
                pdf.multi_cell(180, 7, _s(
                    ...
                    "(Menzies et al., AIDS 2011). All findings are observational and indicative. Prospective "
...
""")
    ...
    for icon, title, duration, description, deliverables in pilot_steps:
        st.markdown(f"""<div style="background:#161b22;border:1px solid #444c56;
            ...
            <p style="color:#cdd9e5;font-size:1rem;margin:0 0 0.75rem 0">{description}</p>
        </div>""", unsafe_allow_html=True)
