...
from constants import (
    ...
    BASELINE_AUC, OUTCOME_NAME_HINTS, CRITICAL_FEATURES, MEDIUM_THRESHOLD,
    TRAINING_MEDIANS,
)
...
with st.sidebar:
    ...
    if MODEL_OK:
        ...
        <div class="metric-box" style="border-color:#3fb950">
            <div class="metric-val" style="color:#3fb950">0.8124</div>
...
if page == "🏠 Home":
    ...
    st.markdown("""<div style="background:linear-gradient(135deg,#0d1117 0%,#161b22 50%,#0d2137 100%);
        ...
        <strong style="color:#21d4fd">21,273 adults on ART</strong> (a Nigerian HIV
    ...
    for col, val, lbl in [
        (c1, "0.8124", "Temporal Validation AUC"),
        (c2, "0.8019", "Cross-Validation AUC"),
        (c3, "21K", "Training Records"),
    ]:
        with col:
    ...
    if MODEL_OK:
        ...
        df_prev['risk_label'] = ['HIGH' if p >= BASELINE_THRESHOLD else 'MEDIUM' if p >= MEDIUM_THRESHOLD else 'LOW'
                                  for p in probs_prev]
...
""")
    ...
    if st.session_state.get('recal_done'):
        ...
        st.info(
            "ℹ️ Using baseline model (Nigerian discovery cohort, AUC: 0.8124). "
    ...
    with c1:
        st.markdown(f'<div class="risk-high"><div class="risk-number">{n_high}</div><div class="risk-label">High Risk &ge;{BASELINE_THRESHOLD*100:.1f}%</div></div>'.replace('&ge;', '\u2265'), unsafe_allow_html=True)
    with c2:
        st.markdown(f'<div class="risk-medium"><div class="risk-number">{n_med}</div><div class="risk-label">Medium Risk {MEDIUM_THRESHOLD*100:.1f}–{BASELINE_THRESHOLD*100:.1f}%</div></div>', unsafe_allow_html=True)
    with c3:
        st.markdown(f'<div class="risk-low"><div class="risk-number">{n_low}</div><div class="risk-label">Low Risk &lt;{MEDIUM_THRESHOLD*100:.1f}%</div></div>', unsafe_allow_html=True)
    with c4:
    ...
    export.insert(3, 'model_auc_temporal', '0.8124')
    ...
    if _scored is not None and len(_scored) > 0:
        ...
        for f in FEATURES:
            if f not in df_shap.columns:
                df_shap[f] = TRAINING_MEDIANS[f]
            else:
                df_shap[f] = pd.to_numeric(df_shap[f], errors='coerce').fillna(TRAINING_MEDIANS[f])
        ...
        if 'risk_label' not in df_shap.columns:
            df_shap['risk_label'] = ['HIGH' if p >= BASELINE_THRESHOLD else 'MEDIUM' if p >= MEDIUM_THRESHOLD else 'LOW'
                                     for p in probs_shap]
...
- **Training data:** 21,273 adults aged 18–100 on ART (a Nigerian HIV programme, Quality of Care dataset — discovery cohort). Poor-adherence prevalence 3.63% (773 patients).
...
- **Cross-validation:** 10-fold stratified CV, natural class distribution — AUC 0.802 (Brier 0.026)
- **Temporal validation:** trained on patients initiating ART up to 14 September 2016, tested on 6,382 later initiators — AUC 0.8124 (95% CI 0.778–0.844, Brier 0.026)
- **Note:** internal and temporal estimates agree (0.802 vs 0.812). That agreement is the point: a model that scores far higher internally than temporally is usually leaking information between training and test.
...
""")
    ...
    st.markdown("""
1. **Temporal validation AUC of 0.8124** — the honest real-world estimate, measured on patients the model never saw.
...
""")
    ...
    if _scored is not None and len(_scored) > 0:
        ...
        for f in FEATURES:
            if f not in df_ie.columns:
                df_ie[f] = TRAINING_MEDIANS[f]
            else:
                df_ie[f] = pd.to_numeric(df_ie[f], errors='coerce').fillna(TRAINING_MEDIANS[f])
        ...
        if 'risk_label' not in df_ie.columns:
            df_ie['risk_label'] = ['HIGH' if p >= BASELINE_THRESHOLD else 'MEDIUM' if p >= MEDIUM_THRESHOLD else 'LOW'
                                   for p in probs_ie]
    ...
    def generate_recommendation(row):
        ...
        # `risk` is risk_pct (0-100), so compare against the threshold * 100.
        # This previously read `risk >= 0.7`, i.e. 0.7% — true for nearly every patient.
        if row['sex_female'] < 0.5 and risk >= BASELINE_THRESHOLD * 100:
        ...
        if not reasons:
            if risk >= BASELINE_THRESHOLD * 100:
        ...
        # Urgency
        # Ladder keyed to the two thresholds actually derived from the temporal
        # holdout (constants.py), not to invented cut-points. The old ladder
        # (90/80/70/40) belonged to the SMOTE-trained model whose scores ran to
        # ~93%; this model's highest score is ~39%, so none of it ever fired and
        # every patient fell through to "Routine follow-up".
        if risk >= BASELINE_THRESHOLD * 100:
            urgency = "Contact within 48 hours"
            urgency_color = "🔴"
        elif risk >= MEDIUM_THRESHOLD * 100:
    ...
    if _scored is not None and len(_scored) > 0:
        ...
        for f in FEATURES:
            if f not in df_ci.columns:
                df_ci[f] = TRAINING_MEDIANS[f]
            else:
                df_ci[f] = pd.to_numeric(df_ci[f], errors='coerce').fillna(TRAINING_MEDIANS[f])
        ...
        if 'risk_label' not in df_ci.columns:
            df_ci['risk_label'] = ['HIGH' if p >= BASELINE_THRESHOLD else 'MEDIUM' if p >= MEDIUM_THRESHOLD else 'LOW'
                                   for p in probs_ci]
...
""")
    ...
    if _scored is not None and len(_scored) > 0:
        ...
        for f in FEATURES:
            if f not in df_dash.columns:
                df_dash[f] = TRAINING_MEDIANS[f]
            else:
                df_dash[f] = pd.to_numeric(df_dash[f], errors='coerce').fillna(TRAINING_MEDIANS[f])
        ...
        if 'risk_label' not in df_dash.columns:
            df_dash['risk_label'] = ['HIGH' if p >= BASELINE_THRESHOLD else 'MEDIUM' if p >= MEDIUM_THRESHOLD else 'LOW'
                                     for p in probs_dash]
    ...
    with c_pts:
        ...
        top5['Urgency'] = top5['risk_pct'].apply(lambda x: '🔴 Today' if x>=BASELINE_THRESHOLD*100 else '🟠 48hrs')
    ...
    with c_actions:
        ...
        urgent = (df_dash['risk_pct']>=BASELINE_THRESHOLD*100).sum()
        ...
        actions_data = [
            ('🔴 URGENT', f'{urgent} patients at HIGH risk', 'Phone contact within 24 hours'),
            ('🟠 HIGH', f'{n_high-urgent} patients at MEDIUM risk', 'Contact within 48 hours'),
...
""")
    ...
    for optimal performance. The temporal AUC (0.8124) is the most realistic deployment estimate.
    ...
    c1, c2, c3, c4, c5 = st.columns(5)
    cv_metrics = [
        (c1, "0.8019",  "CV AUC",        "#21d4fd"),
        (c2, "21,273", "Adults",        "#21d4fd"),
    ...
    st.markdown("""<div style="font-size:0.72rem;color:#8b949e;padding:0.3rem 0.25rem 0.1rem 0.25rem">
        AUC 0.802 (SD 0.024) across 10 stratified folds of 21,273 adults, evaluated at the
    ...
    st.markdown("""<div style="font-size:0.72rem;color:#8b949e;padding:0.3rem 0.25rem 0.1rem 0.25rem">
        Temporal AUC 0.8124 (95% CI: 0.778–0.844) on 6,382 patients who started ART after the training window.
    ...
    st.markdown("""
**Cross-validation AUC (0.802)** is measured with 10-fold stratified CV across 21,273 adults, at
...
**Temporal AUC (0.8124)** measures how well the model predicts on patients from *later time periods*
it has never seen — trained on those initiating ART up to September 2016, tested on 6,942 who
started after. AUC 0.8124 means the model correctly ranks 81.2% of patient pairs (higher-risk above
lower-risk).
...
""")
    ...
    The model was trained on a public Nigerian HIV programme dataset (Quality of Care dataset; 21,273 adults after cleaning; ART initiations 2013–2017 for 99.8% of the cohort — discovery cohort). The deposit does not document its sampling frame, custodian, or collection methodology.
