...
# ═════════════════════════════════════════════════════════════
    ...
    with c_e:
        ...
        ax.set_title(f'Clinical Feature Contributions: {sel_id} | Risk: {pt_prob:.1f}% ({pt_lbl})',
...
""")
    ...
    if len(df_high) > 0 and len(df_low) > 0:
        ...
        for i, row in enumerate(comp_df.itertuples()):
            ax.text(i - w/2, row._3 + 0.5, f"{row._3:.0f}%", ha='center',
                    fontsize=8, color='#e6edf3', fontweight='bold')
            ax.text(i + w/2, row._4 + 0.5, f"{row._4:.0f}%", ha='center',
                    fontsize=8, color='#e6edf3', fontweight='bold')
