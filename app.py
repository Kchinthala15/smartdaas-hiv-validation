...
""")
    ...
    if _dhis2_df is not None and not use_demo and uploaded is None:
        try:
    ...
    elif uploaded is not None and not use_demo:
        try:
    ...
    # Make the scored cohort available to the Outreach Optimiser page.
    ...
    if st.button("📄 Generate Executive PDF Report", use_container_width=True, type="primary"):
        with st.spinner("Generating professional PDF report..."):
            try:
                ...
                pdf.set_font('Helvetica', '', 7); pdf.set_text_color(0,171,187)
                pdf.set_xy(60, 25); pdf.cell(90, 6, 'HIV PROGRAMME INTELLIGENCE')
                pdf.set_font('Helvetica', 'B', 10); pdf.set_text_color(*_MUTED)
                pdf.set_xy(128, 12); pdf.cell(74, 7, 'Programme Intelligence', align='R')
                pdf.set_font('Helvetica', '', 8)
                pdf.set_xy(128, 21); pdf.cell(74, 6, _s(f'Executive Report  .  {report_date.strftime("%Y")}'), align='R')
                ...
                pdf.set_xy(15, 66); pdf.cell(180, 7, 'Predictive Risk  .  Facility Analytics  .  Economic Impact  .  Recommended Actions')
                ...
                for i, (lbl, val) in enumerate(cover_details):
                    pdf.set_font('Helvetica', '', 9.5); pdf.set_text_color(*_MUTED)
                    pdf.set_xy(22, 82+i*9); pdf.cell(42, 8, _s(lbl+':'))
                    pdf.set_font('Helvetica', 'B', 9.5); pdf.set_text_color(*_WHITE)
                    pdf.cell(128, 8, _s(val))

                # Snapshot header
                pdf.set_font('Helvetica', 'B', 9); pdf.set_text_color(*_CYAN)
                pdf.set_xy(15, 122); pdf.cell(180, 6, 'PROGRAMME SNAPSHOT')
                pdf.set_draw_color(*_CYAN); pdf.set_line_width(0.3)
                ...
                for i, (val, lbl, vc) in enumerate(snap_cards):
                    ...
                    pdf.set_font('Helvetica', 'B', 7); pdf.set_text_color(*_WHITE)
                    pdf.set_xy(cx, cy+13); pdf.cell(cw, 6, lbl, align='C')
                ...
                pdf.set_font('Helvetica', 'B', 7.5); pdf.set_text_color(*_WHITE)
                pdf.set_xy(15, by_r+10)
                pdf.cell(high_w, 6, _s(f'{n_high} HIGH  {pct_high:.1f}%'), align='C')
                pdf.cell(med_w,  6, _s(f'{n_med} MED'),                     align='C')
                pdf.cell(low_w,  6, _s(f'{n_low} LOW  {n_low/n_total*100:.1f}%'), align='C')

                # What's inside
                pdf.set_font('Helvetica', 'B', 11); pdf.set_text_color(*_CYAN)
                pdf.set_xy(15, 180); pdf.cell(180, 8, "WHAT'S INSIDE THIS REPORT")
                pdf.set_draw_color(*_CYAN); pdf.set_line_width(0.4)
                ...
                contents = [
                    ...
                    ('04','Recommended Programme Actions', 'Immediate, short-term and strategic interventions'),
                    ('05','Methodology & Limitations',     'Model validation, economic assumptions and data governance'),
                ]
                for i, (num, title, desc) in enumerate(contents):
                    y = 191 + i*15
                    pdf.set_fill_color(*_CYAN); pdf.rect(15, y+1, 11, 11, 'F')
                    pdf.set_font('Helvetica', 'B', 8); pdf.set_text_color(*_BG)
                    pdf.set_xy(15, y+2.5); pdf.cell(11, 8, num, align='C')
                    ...
                    pdf.set_xy(30, y+8); pdf.cell(165, 6, _s(desc))
                ...
                pdf.set_xy(21, 272); pdf.set_font('Helvetica', 'B', 8.5)
                pdf.set_text_color(*_AMBER); pdf.cell(30, 5, 'IMPORTANT')
                pdf.set_font('Helvetica', 'I', 8); pdf.set_text_color(*_TEXT)
                pdf.set_xy(21, 278)
                pdf.multi_cell(168, 5, _s('SmartDaaS v1.0 is a decision-support platform for HIV programme intelligence. Not intended to replace clinical judgment. All outputs require review by qualified programme teams before operational use. Local validation required before deployment.'))
                ...
                pdf.set_font('Helvetica', 'B', 11); pdf.set_text_color(*_WHITE)
                pdf.set_x(15); pdf.cell(180, 8, 'Programme Risk Overview', ln=True)
                pdf.ln(2)
                ...
                exec_metrics = [
                    ...
                    ('Estimated Avoidable Programme Cost',   f'USD {est_avoidable_cost:,} (illustrative)', _CYAN),
                ]
                for label, value, vc in exec_metrics:
                    pdf.set_fill_color(*_CARD)
                    pdf.set_font('Helvetica', '', 10); pdf.set_text_color(*_MUTED)
                    pdf.set_x(15); pdf.cell(125, 8, _s('  '+label), fill=True)
                    pdf.set_font('Helvetica', 'B', 10); pdf.set_text_color(*vc)
                    pdf.cell(55, 8, value, fill=True, ln=True)
                    pdf.ln(1)

                pdf.ln(4)
                pdf.set_font('Helvetica', 'B', 11); pdf.set_text_color(*_WHITE)
                pdf.set_x(15); pdf.cell(180, 8, 'Key Clinical Findings', ln=True)
                ...
                for finding in findings:
                    pdf.set_font('Helvetica', '', 10); pdf.set_text_color(*_TEXT)
                    pdf.set_x(15); pdf.cell(7, 7, chr(149))
                    pdf.multi_cell(163, 7, finding); pdf.ln(1)
                ...
                pdf.set_font('Helvetica', '', 10); pdf.set_text_color(*_MUTED)
                pdf.set_x(15)
                pdf.multi_cell(180, 6, _s('Based on Paper 2 analysis of 27,288 patients from the Nigerian national HIV programme (Chinthala 2026, submitted to BMJ Global Health).'))
                pdf.ln(4)
                ...
                pdf.set_font('Helvetica', '', 7); pdf.set_text_color(*_MUTED)
                for val, lbl in [(0.7,'0.7'),(1.0,'1.0'),(1.5,'1.5'),(2.0,'2.0'),(2.6,'2.6')]:
                    sx = scale_start + scale_w*(val-0.7)/(2.8-0.7)
                    pdf.set_xy(sx-4, plot_y-5); pdf.cell(10, 5, lbl, align='C')
                ...
                for i, (lbl, OR, lo, hi, color) in enumerate(findings_data):
                    ...
                    pdf.set_xy(17, fy+4); pdf.cell(58, 8, lbl)
                    pdf.set_text_color(*color)
                    pdf.set_xy(77, fy+4); pdf.cell(13, 8, f'{OR:.2f}')
                ...
                pdf.set_font('Helvetica', 'I', 8); pdf.set_text_color(*_MUTED)
                pdf.set_x(15)
                pdf.cell(180, 6, _s('Square = OR estimate. Line = 95% CI. Dashed line = null (OR 1.0).'))
                pdf.ln(5)
                ...
                for finding_title, stat, desc, action in facility_findings:
                    pdf.set_fill_color(*_CARD)
                    pdf.set_font('Helvetica', 'B', 10); pdf.set_text_color(*_CYAN)
                    pdf.set_x(15); pdf.cell(180, 8, _s(f'  {finding_title}: {stat}'), fill=True, ln=True)
                    pdf.ln(1)
                    pdf.set_font('Helvetica', '', 10); pdf.set_text_color(*_TEXT)
                    pdf.set_x(15); pdf.multi_cell(180, 6, _s(f'  Finding: {desc}'))
                    pdf.set_text_color(*_GREEN)
                    pdf.set_x(15); pdf.multi_cell(180, 6, _s(f'  Action: {action}'))
                    pdf.ln(3)
                ...
                pdf.set_x(15); pdf.cell(180, 7, 'Patients requiring immediate contact and adherence support.', ln=True)
                pdf.ln(3)
                ...
                pdf.set_fill_color(*_CARD)
                pdf.set_font('Helvetica', 'B', 9); pdf.set_text_color(*_CYAN)
                t_cols = [('Patient ID',28),('Risk %',20),('Age',12),('CD4',18),
                          ('Stage',16),('Prior Int.',20),('Top Driver',42),('Action',30)]
                pdf.set_x(15)
                for col_name, width in t_cols:
                    pdf.cell(width, 10, col_name, fill=True)
                pdf.ln()
                pdf.set_draw_color(*_CYAN); pdf.set_line_width(0.3)
                ...
                for _, row in top10.iterrows():
                    ...
                    cells = [
                        (str(row['patient_id']), 28, _TEXT),
                        (f"{row['risk_pct']:.1f}%", 20, _RED),
                        (f"{row['Age']:.0f}", 12, _TEXT),
                        (f"{row['Cd4AtStart']:.0f}", 18, _TEXT),
                        (f"Stage {row['stage_start_num']:.0f}", 16, _TEXT),
                        ("Yes" if row['had_interruption'] > 0.5 else "No", 20, _TEXT),
                        (str(row['top_driver'])[:28], 42, _AMBER),
                        (urgency, 30, _AMBER),
                    ]
                    pdf.set_x(15)
                    for val, width, vc in cells:
                        pdf.set_font('Helvetica', '', 9); pdf.set_text_color(*vc)
                        pdf.cell(width, 8, val)
                    pdf.ln()
                ...
                try:
                    ...
                    y_box = pdf.get_y() + 4
                    pdf.set_xy(20, y_box)
                    pdf.set_font('Helvetica', 'B', 11); pdf.set_text_color(*_RED)
                    pdf.cell(50, 7, _s(f"{top_pat['risk_pct']:.1f}% — HIGH RISK"))
                    pdf.set_font('Helvetica', '', 10); pdf.set_text_color(*_TEXT)
                    pdf.cell(50, 7, _s(f"Patient: {top_pat['patient_id']}"))
                    pdf.cell(40, 7, _s(f"Age: {top_pat['Age']:.0f}"))
                    pdf.cell(40, 7, _s(f"CD4: {top_pat['Cd4AtStart']:.0f} cells/uL"))
                    pdf.ln(14)
                    ...
                    fig, ax = plt.subplots(figsize=(9, 5.5), facecolor='#0d1117')
                    ax.set_facecolor('#0d1117')
                    bars = ax.barh(range(len(sv_names)), sv_vals,
                                   color=sv_colors, height=0.65,
                                   edgecolor='#161b22', linewidth=0.3)
                    for i, (bar, v) in enumerate(zip(bars, sv_vals)):
                        x_pos = v + 0.001 if v >= 0 else v - 0.001
                        ha = 'left' if v >= 0 else 'right'
                        ax.text(x_pos, i, f'{v:+.4f}', va='center', ha=ha,
                                fontsize=8, color='#e6edf3')
                    ax.set_yticks(range(len(sv_names)))
                    ax.set_yticklabels(sv_names, fontsize=8.5, color='#cdd9e5')
                    ax.axvline(0, color='#8b949e', lw=1.2)
                    ...
                    ax.set_title(
                        ...
                        color='#e6edf3', fontsize=10, pad=10, fontweight='bold'
                    )
                    plt.tight_layout()
                    ...
                    for rank, feat_idx in enumerate(sv_desc_order[:3], 1):
                        ...
                        pdf.set_font('Helvetica', '', 10); pdf.set_text_color(*_TEXT)
                        pdf.set_x(20)
                        pdf.cell(6, 7, _s(f'{rank}.'))
                        pdf.multi_cell(169, 7, _s(
                            f"{label} {direction} (SHAP: {sv_val:+.4f})"
                        ))
                    pdf.ln(3)
                    pdf.set_font('Helvetica', 'I', 8); pdf.set_text_color(*_MUTED)
                    pdf.set_x(15)
                    pdf.multi_cell(180, 5, _s(
                ...
                for sec_title, color, actions in action_sections:
                    ...
                    for action in actions:
                        pdf.set_font('Helvetica', '', 10); pdf.set_text_color(*_TEXT)
                        pdf.set_x(20); pdf.cell(6, 7, chr(149))
                        pdf.multi_cell(164, 7, _s(action)); pdf.ln(1)
                    pdf.ln(5)
                ...
                pdf.multi_cell(180, 7, _s(
                    ...
                    "from the Nigerian national HIV programme (discovery cohort). Cross-validation AUC: 0.963; "
                    "temporal validation AUC: 0.772. SHAP (SHapley Additive exPlanations) values provide "
                    "per-patient clinical reasoning. Facility intelligence is based on multivariable logistic "
                    "regression with HC3 cluster-robust standard errors applied to 27,288 Nigerian HIV programme "
                    "patients (Chinthala 2026). Economic estimates use USD 1,850 per averted poor outcome "
                    "(Menzies et al., AIDS 2011) with a conservative 23% interruption reduction assumption "
                    "for contacted high-risk patients (PEPFAR retention literature). All findings are illustrative. Prospective "
                    "validation is required before programmatic application. SmartDaaS v1.0 is a decision-support "
                    "platform for HIV programme intelligence. All outputs require review by qualified programme "
                    "teams prior to operational use. Code: github.com/Kchinthala15/smartdaas-hiv-validation"
                ))
                pdf.ln(8)
                # Data source note
                ...
                pdf.set_font('Helvetica', 'B', 9); pdf.set_text_color(*_CYAN)
                pdf.set_xy(20, pdf.get_y()+2); pdf.cell(170, 6, 'DATA SOURCE')
                pdf.set_font('Helvetica', '', 9); pdf.set_text_color(*_TEXT)
                pdf.set_xy(20, pdf.get_y()+6); pdf.cell(170, 6, _s(data_source))
