    ...
    if st.button("📄 Generate Executive PDF Report", use_container_width=True, type="primary"):
        with st.spinner("Generating professional PDF report..."):
            try:
                ...
                pdf.set_font('Helvetica', 'B', 12); pdf.set_text_color(*_MUTED)
                pdf.set_xy(128, 12); pdf.cell(94, 7, 'Programme Intelligence', align='R')
                pdf.set_font('Helvetica', '', 10)
                pdf.set_xy(128, 21); pdf.cell(74, 8, _s(f'Executive Report  .  {report_date.strftime("%Y")}'), align='R')
                ...
                pdf.set_font('Helvetica', 'B', 10); pdf.set_text_color(*_WHITE)
                pdf.set_xy(15, by_r+10)
                pdf.cell(high_w, 8, _s(f'{n_high} HIGH  {pct_high:.1f}%'), align='C')
                pdf.cell(med_w,  8, _s(f'{n_med} MED'),                     align='C')
                pdf.cell(low_w,  8, _s(f'{n_low} LOW  {n_low/n_total*100:.1f}%'), align='C')
                ...
                for i, (num, title, desc) in enumerate(contents):
                    ...
                    pdf.set_font('Helvetica', 'B', 10); pdf.set_text_color(*_BG)
                    pdf.set_xy(15, y+2.5); pdf.cell(11, 10, num, align='C')
                    pdf.set_font('Helvetica', 'B', 13); pdf.set_text_color(*_WHITE)
                    pdf.set_xy(30, y+1); pdf.cell(165, 9, title)
                    pdf.set_font('Helvetica', '', 11); pdf.set_text_color(*_MUTED)
                    pdf.set_xy(30, y+8); pdf.cell(185, 6, _s(desc))
                ...
                for finding_title, stat, desc, action in facility_findings:
                    ...
                    pdf.set_x(15); pdf.cell(1100, 8, _s(f'  {finding_title}: {stat}'), fill=True, ln=True)
                    pdf.ln(1)
                    pdf.set_font('Helvetica', '', 12); pdf.set_text_color(*_TEXT)
                    pdf.set_x(15); pdf.multi_cell(180, 8, _s(f'  Finding: {desc}'))
                    pdf.set_text_color(*_GREEN)
                    pdf.set_x(15); pdf.multi_cell(180, 8, _s(f'  Action: {action}'))
                    pdf.ln(3)
                ...
                try:
                    ...
                    min_bar_for_inline = x_range * 0.12
                    for i, (bar, v) in enumerate(zip(bars, sv_vals)):
                        ...
                        if abs_v >= min_bar_for_inline:
                            ...
                            # Short bar — always place annotation to the right of zero
                            x_pos = x_range * 0.03
                            ha = 'left'
