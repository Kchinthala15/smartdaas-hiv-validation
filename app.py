...
# ═════════════════════════════════════════════════════════════
    ...
    Each factor is shown with its exact contribution — red increases risk, green reduces it.
    """)
    ...
    # Load demo data for patient selection
    rng = np.random.RandomState(42)
    idx = rng.choice(len(X_DEMO), min(200, len(X_DEMO)), replace=False)
    ...
    with c1:
        risk_filter = st.selectbox("Filter by risk tier:",
                                   ["All patients","HIGH only","MEDIUM only","LOW only"])
    with c2:
        sort_by = st.selectbox("Sort by:",
                               ["Highest risk first","Lowest risk first","Patient ID"])
    ...
    pid_list = df_filtered['patient_id'].tolist()
    sel_id   = st.selectbox(
        f"Select patient ({len(pid_list)} shown):",
        pid_list,
        index=0
    )
    ...
    with c3:
        ...
        top3_idx = np.argsort(np.abs(pt_sv))[-3:][::-1]

        feat_list = list(FEATURES)
        top3_list = [int(x) for x in top3_idx]
        pt_sv_list = [float(x) for x in pt_sv]
        drivers = []
        for ii in top3_list:
            feat_name = FEAT_LABELS.get(feat_list[ii], feat_list[ii])
    ...
    # ── WATERFALL CHART ───────────────────────────────────
    st.markdown('<p class="section-hdr">SHAP Waterfall — Feature Contributions</p>',
                unsafe_allow_html=True)
    st.markdown("Each bar shows how much that feature pushed the risk score **up (red)** or **down (green)** from the average prediction.")

    # Sort by absolute SHAP value
    sv_order  = np.argsort(np.abs(pt_sv))
    sv_vals   = [float(pt_sv[int(x)]) for x in sv_order]
    sv_names  = [FEAT_LABELS.get(FEATURES[int(i)], FEATURES[int(i)]) for i in sv_order]
    sv_colors = ['#f85149' if float(v) > 0 else '#3fb950' for v in sv_vals]

    # Add patient feature values to labels
    feat_list2 = list(FEATURES)
    sv_order_list = [int(x) for x in sv_order]
    sv_vals_list = [float(x) for x in pt_sv]
    sv_labels = []
    for ii, name in zip(sv_order_list, sv_names):
        val = float(sel_row[feat_list2[ii]])
        sv_labels.append(f"{name} = {val:.1f}")

    fig, ax = plt.subplots(figsize=(9, 6), facecolor='#161b22')
    ax.set_facecolor('#161b22')

    bars = ax.barh(range(len(sv_labels)), sv_vals,
                   color=sv_colors, height=0.7,
    ...
    for i, (bar, v) in enumerate(zip(bars, sv_vals)):
        ...
        ax.text(x_pos, i, f'{v:+.4f}',
                va='center', ha=ha, fontsize=8.5, color='#e6edf3')

    ax.set_yticks(range(len(sv_labels)))
    ax.set_yticklabels(sv_labels, fontsize=9, color='#e6edf3')
    ...
    for sp in ax.spines.values(): sp.set_color('#30363d')
    ax.set_title(
        f'SHAP Explanation: {sel_id} | Risk Score: {pt_prob:.1f}% ({pt_lbl})',
        color='#e6edf3', fontsize=11, pad=12, fontweight='bold'
    )

    # Add baseline annotation
    baseline = probs_shap.mean() * 100
    ax.text(0.99, 0.02, f'Baseline (cohort avg): {baseline:.1f}%',
    ...
    st.markdown('<p class="section-hdr">Full Clinical Profile</p>',
                unsafe_allow_html=True)

    profile_data = []
    feat_list3 = list(FEATURES)
    pt_sv_full = [float(x) for x in pt_sv]
    for feat in feat_list3:
        val    = sel_row[feat]
        sv_idx = feat_list3.index(feat)
        sv_val = pt_sv_full[sv_idx]
    ...
    # ── RECOMMENDED ACTIONS ───────────────────────────────
    st.markdown('<p class="section-hdr">Recommended Clinical Actions</p>',
                unsafe_allow_html=True)
    for action in INTERVENTIONS.get(pt_lbl, INTERVENTIONS['LOW']):
        st.markdown(action)

    # ── EXPORT ────────────────────────────────────────────
    st.markdown('<p class="section-hdr">Export Patient Explanation</p>',
                unsafe_allow_html=True)
