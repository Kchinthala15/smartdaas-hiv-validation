    ...
    if st.button("📄 Generate Executive PDF Report", use_container_width=True, type="primary"):
        with st.spinner("Generating professional PDF report..."):
            try:
                ...
                pdf.cell(high_w, 8, _s(f'{n_high} HIGH {pct_high:.1f}%') if high_w > 20 else '', align='C')
                pdf.cell(med_w,  8, _s(f'{n_med} MED') if med_w > 15 else '',  align='C')
                pdf.cell(low_w,  8, _s(f'{n_low} LOW {n_low/n_total*100:.1f}%') if low_w > 20 else '', align='C')
