...
""")
    ...
    if uploaded is not None and not use_demo:
        # ── DUA gate — must acknowledge before processing ──
        if not render_dua_gate(context="upload"):
            st.stop()
...
""")
    ...
    # DUA gate for real uploads
    if uploaded_rep is not None and not use_demo_rep:
        if not render_dua_gate(context="report"):
            st.stop()
