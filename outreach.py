...
from constants import (
    FEATURES, COST_PER_POOR_OUTCOME, FEAT_LABELS,
    BASELINE_THRESHOLD, MEDIUM_THRESHOLD,
)
...
def get_secret(key, default=None):
    """
    Read a secret from st.secrets (Streamlit Cloud) or os.environ
    (Render/Docker), with graceful fallback.

    Defined here rather than imported: the original lives in app.py, and
    app.py imports this module, so importing it back would be circular.
    It was being called at three sites in this file without ever being in
    scope -- a NameError on every call.
    """
    try:
...
def _oo_demo_cohort(n=200, seed=42):
    ...
    def _block(size, rmean, rstd, stage_high=False):
        risk = np.clip(rng.normal(rmean, rstd, size), 0.001, 0.45)
        return pd.DataFrame({
    ...
    # Block means are placed on the deployed model's score scale, which runs to
    # ~0.39 with a median near 0.02. They previously sat at 0.82 / 0.52 / 0.18 —
    # the scale of the SMOTE-trained model, whose probabilities were calibrated
    # to a 50% prevalence. Demo proportions are deliberately risk-heavy so the
    # outreach plan has something to show; they are not a realistic case mix.
    parts = [_block(n_high, 0.22, 0.05, True), _block(n_medium, 0.11, 0.02),
             _block(n_low, 0.03, 0.02)]
    idx = 0
    for p in parts:
    ...
    df['risk_label'] = pd.cut(df['risk_score'],
                              bins=[-0.001, MEDIUM_THRESHOLD, BASELINE_THRESHOLD, 1.001],
