...
def _render_detail(patient_id):
    ...
    if submitted:
        ...
        # Also write to the shared audit_log table via the app's central
        # log_event(), same pattern as every other page. No-ops safely if
        # supabase/log_event weren't passed in (e.g. local/dev use).
        supabase = st.session_state.get("al_supabase")
        log_fn = st.session_state.get("al_log_event")
        if supabase is not None and log_fn is not None:
            log_fn(supabase, "actionlens_outreach", {
                "patient_id": str(patient_id),
                "risk_category": entry["risk_category_shown"],
                "language": entry["language"],
                "outreach_attempted": entry["outreach_attempted"],
...
def render_action_lens(supabase=None, log_event=None):
    ...
    Args:
        supabase: optional Supabase client, passed through from app.py.
                  When provided, outcome logs are also written to the
                  shared 'audit_log' table (same table/pattern used by
                  every other page in app.py). Fails silently if None
                  or if the write fails, exactly like the rest of the app.
        log_event: optional log_event(supabase, event_type, payload)
                  function, passed through from app.py, for consistent
                  central audit logging.
    """
    st.session_state["al_supabase"] = supabase
    st.session_state["al_log_event"] = log_event
